...
author: "Rasmus Andsbjerg Johansen"
...
ggplot(data = Western_Europe) + 
  ...
  labs(title = "Long-Term Trend in Homicide Rates in Western Europe",
       x = "Year",
...
```{r rule-duration, eval = FALSE}
Kongerække <- read_csv2("data/Kongerækken-csv.csv")
Kongerække <- mutate(Kongerække, midyear=Kongerække$`Regering slut`-(Kongerække$`Regering slut`- Kongerække$`Regerings start`)/2, duration= Kongerække$`Regering slut`- Kongerække$`Regerings start`)

ggplot(data=Kongerække, aes(x=midyear, y=duration))+
  geom_smooth()+
  labs(title = "Kongerækken", y="Regeringstid" ,x="År")
...
The charts show that murders have gone down in Western Europe from the Middle Ages to the 20th century. This suggests our society has become more peaceful and less violent over time, indicating we might be more civilized now. There are several reasons for this drop in murder rates.

First, our laws and police forces have improved in handling crime and maintaining order. This has made our societies more stable and safe. But there are other factors too. Better healthcare has helped people live longer and healthier lives, which might reduce violent situations. Also, societal norms and values have changed. We now value human rights, education, and social welfare more. This has created a culture where we respect each other's lives and solve conflicts without violence.

Economic growth and urbanization have also changed our societies. When people become more interdependent and cooperative, there's less reason to be violent. Democratic institutions and the rule of law have given us peaceful ways to solve problems, which has also helped reduce violence.

While these changes are positive, we should remember that other forms of violence still exist. For example, domestic violence and cybercrime might have become more common in recent decades. So, even though we can be happy about the decrease in murder rates, we still need to work on addressing the causes of violence and creating a more just and peaceful society.
